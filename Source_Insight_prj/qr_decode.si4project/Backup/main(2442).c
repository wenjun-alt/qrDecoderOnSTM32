...
//u8 qrLocationFlag = 0;  				//qr码的位置信息标识

//摄像头数据DMA接收完成中断回调函数 --> 双缓冲中断机制
//设置DMA搬完从DCMI收到的一帧数据(DCMI开启了帧中断),
//中的一行图像数据(双缓冲中断机制)则触发DMA中断,并调用此回调函数
void qr_dcmi_rx_callback(void)
{  
	u32 *pbuf;
	u16 i;
	/*将rgb_data_buf地址偏移赋值给pbuf,即利用该局部变量访问堆上的rgb_data_buf
	  这里就利用双缓存机制一行一行的将图像数据拷贝到RGB565帧缓存中
	*/
	pbuf=(u32*)(rgb_data_buf+dcmi_curline*qr_image_width);//rgb_data_buf是2个字节移动一次

	/* 当前目标存储器为存储器1 */
	// 通过查看DMA_SxCR寄存器的bit19：CT这个位来判断当前DMA传输的目标存储器
	if(DMA2_Stream1->CR&(1<<19))//DMA使用buf1,读取buf0
	{ 
		for(i=0;i<qr_image_width/2;i++)//为啥要除2,因为dcmi_line_buf是4个字节一搬,即一次拷贝2个pix
		{
			pbuf[i]=dcmi_line_buf[0][i];//搬到外部SRAM中
		} 
	}
	/* 当前目标存储器为存储器0 */
...
//imagebuf:RGB图像数据缓冲区
//解码函数
/*传入的实参是：qr_image_width->imagewidth:摄像头采集的输入图像尺寸(这里只是指图像的像素大小),
该图像的大小在main函数中根据LCD分辨率设置OV2640的图像尺寸,
这里实际最大只能为480,最小240,这里采用的是两块LCD屏幕来做判断的
rgb_data_buf->imagebuf:存放rgb565这一帧输入图像真正需要的内存大小,即需要多少个字节
*/
void qr_decode(u16 imagewidth,u16 *imagebuf)
{
	...
	for(i=0; i < qr_img_width; i++)		
	{
		...
		{	
		/*如何理解这里的图像压缩？
		imagebuf+((i*imagewidth)+j)*qr_img_scale，这里是在地址偏移，换句话来说我在做图像的pix偏移
		我每次让它相对于原输入图像偏移多少个像素取一个像素点
		举个例子：
		比如：这里我的输入图像为480*480,即imagewidth = 480,表示这里一行有480个像素
		假设一帧图像的起始地址imagebuf=0,表示第0个像素点,(一个像素占2个字节)
		当前行为i=0,列为j=0,我取出第0个pix的数据,
		下一次当前行为i=0,列为j=1,即(imagebuf+1*2)->表示我取出第2个pix的数据,
		这里就可以看出我在对图像进行压缩了,为什么？
		因为我每次都是隔一个pix取出它的图像数据
		*/
...
{						 
	...
	W25QXX_Init();				//初始化外部flash(W25Q128->128Mb->16MB),内部Flash->1MB
	...
	OV2640_Special_Effects(0);		//摄像头特效设置-->普通模式
	OV2640_RGB565_Mode();			//图片格式设置-->RGB565模式

/*	1.初始化DCMI接口,配置全帧捕获,8位数据并口传输等,
	2.开启DCMI的帧中断,使能DCMI接口
	3.等待后续开启DCMI图像捕获
*/
	My_DCMI_Init();	//DCMI配置
...
/*
	1.为图像接收开了两个行缓存line_buffer
	2.为rgb565的一帧缓存图像申请内存(外部的1MB的SRAM),用于存放一帧RGB565图像数据
	3.这些缓存都放在堆上，因为要和解码函数以及中断回调函数配合->由自己实现的内存管理函数进行内存管理
*/

	//图像的一行占用480字节,一帧图片占用112KB
	dcmi_line_buf[0]=mymalloc(SRAMIN,qr_image_width*2);	//为行缓存接收申请内存	
	dcmi_line_buf[1]=mymalloc(SRAMIN,qr_image_width*2); //为行缓存接收申请内存
	//为rgb帧缓存申请内存：像素*像素*2字节(RGB565格式)(外部SRAM),用于存放一帧RGB565图像数据
	//qr_image_width: 根据屏幕分辨率设置OV2640的图像尺寸即qr_image_width
	
	rgb_data_buf=mymalloc(SRAMEX,qr_image_width*qr_image_width*2);//一帧图片112KB			
	
	dcmi_rx_callback=qr_dcmi_rx_callback;//DMA数据接收中断回调函数
	//DMA行传输,一行有240像素,因为是RGB565格式数据,所以一行有240/2=120字节
	DCMI_DMA_Init(
									(u32)dcmi_line_buf[0],(u32)dcmi_line_buf[1],	\
									qr_image_width/2,DMA_MemoryDataSize_HalfWord,DMA_MemoryInc_Enable											
							 );//DCMI DMA配置

	//根据屏幕分辨率设置OV2640的图像尺寸
	OV2640_OutSize_Set(qr_image_width,qr_image_width); 
/*	
	1. 开启DCMI启动传输,包括了LCD显示(写GRAM)
	2. 开启DMA2控制器的数据流1(每个数据流都有一个仲裁器(优先级0 > 优先级1),一个数据流可响应8个数据通道(请求))
	3. 开启DCMI捕获使能,全帧捕获图像
*/
	DCMI_Start();
	...
	{	
		...
		if(readok==1)			//摄像头已经捕获到了一帧图像
		{		
			readok=0;
			/* 利用WIFI-ESP8266进行串口透传发送到服务器 */
			LCD_Color_Fill( (lcddev.width-qr_image_width)/2,(lcddev.height-qr_image_width)/2,	\
											...
											rgb_data_buf );//显示图像
			/* 后面改进，可以做成轮询模式,将解码放到后面 */
			qr_decode(qr_image_width,rgb_data_buf);//识别图像
			
		}
#if WIFI_EN	
/*在串口中断接收中定义标志变量qrLocationFlag，当接收到位置信息后，在main函数中调用解码函数*/
		//if(//qrLocationFlag == 1)
		{
			//1. 对接收到的qr码的位置信息字符串进行解析,提取出每个qr码的位置信息
			//2. 根据解析后的每个位置信息进行图像分割,假设有一张图片上有5个qr码,就要分割出5张图片.
			//3. 挨个进行解码,来个for(int i = 0; i < 5; i++){调用解码函数依次解码...}		
		}
